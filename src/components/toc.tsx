...
function useActiveItem(itemIds: string[]) {
  ...
  React.useEffect(() => {
    const elements = itemIds.map((id) => document.getElementById(id)).filter((el): el is HTMLElement => !!el)
        ...
        {tree.items.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.items?.length ? (
                <Tree tree={item} level={level + 1} activeItem={activeItem} itemIds={itemIds} />
              ) : null}
            </motion.li>
          )
        })}
