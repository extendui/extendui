...
const checkForChildComponent = (
  children: React.ReactNode,
  componentType: React.FC<any> | React.ForwardRefExoticComponent<any>,
) => {
...
const BannerDescription = React.forwardRef<HTMLElement, BannerDescriptionProps>(
  (
    { className, position = 'center', asChild = false, children, ...props },
    ref,
  ) => {
      ...
      const validProps = Object.fromEntries(
        Object.entries(props).filter(([key]) =>
          React.isValidElement(
            key as keyof React.AnchorHTMLAttributes<HTMLAnchorElement>,
          ),
        ),
      );
  ...
  ({ className, onClick, icon = <X size={16} />, ...props }, ref) => {
    ...
    return (
      <button
        ...
        onClick={handleClick}
        className={cn(
          'absolute top-1/2 right-2 -translate-y-1/2 rounded-full p-1.5 transition-all',
          'opacity-70 hover:bg-black/10 hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-none',
          className,
        )}
        aria-label="Dismiss"
        {...props}
