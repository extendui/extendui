...
const calendarVariants = cva(
  'p-3',
  {
    variants: {
      variant: {
        default: '',
        glowingRing: 'relative overflow-hidden ring-2 ring-primary/50 animate-glow transition-all',
      ...
      {months.map((monthName, index) => (
        <button
          ...
          className={cn(
            ...
            "px-2 py-1 text-sm w-full",
            month && month.getMonth() === index && "bg-primary text-primary-foreground"
          )}
        >
  ...
  const YearPicker = () => (
    <div className="grid grid-cols-3 gap-2">
      {yearRange?.map((year) => (
        <button
          ...
          className={cn(
            buttonVariants({ variant: "ghost" }),
            "px-2 py-1 text-sm w-full",
            month && month.getFullYear() === year && "bg-primary text-primary-foreground"
          )}
        >
          {year}
        </button>
      ...
      <div className="flex justify-between">
        <Button size={"icon"} variant={"ghost"} onClick={() => setYearOffset(prev => prev - 12)}><ChevronLeftIcon /></Button>
        <Button size={"icon"} variant={"ghost"} onClick={() => setYearOffset(prev => prev + 12)}><ChevronRightIcon /></Button>
      </div>
    </div>
  )
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        day_today: "text-accent-foreground bg-muted relative after:absolute after:bottom-1 after:left-1/2 after:-translate-x-1/2 after:w-1 after:h-1 after:bg-emerald-600 after:rounded-full",
        day_outside:
